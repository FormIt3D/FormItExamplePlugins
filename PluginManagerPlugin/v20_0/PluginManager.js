...
//This pluginmanager really needs to be cleaned up. Shimming in ES6 functionality because
//this plugin loads plugin code into the main app. Not trying to refactor that right now.
...
FormItExamplePlugins.PluginManager.MakePluginDiv = function(plugin)
{
    ...
    //Hopefully this is sufficient. Plugin manager only knows about the root path,
    //but the applications install plugins which could be versioned.
    //So do a check against just the root path which would match for versioned plugins.
    //For example https://myPlugin/ should match for https://myPlugin/v20/
    var isInstalled = function (path) {
        return !!FormItExamplePlugins.PluginManager.InstalledPlugins.find(function(plugin){
            return plugin.startsWith(path);
        });
    }

    checkboxElem.checked = isInstalled(pluginLocation);

    checkboxElemDiv.onclick = function(e)
    {
        console.log('isinstalled', isInstalled(pluginLocation))

        if (isInstalled(pluginLocation))
        {
